...
const PropertyList: React.FC<IPropertyList> = ({ lang, initialData, metaData }) => {
    ...
    const handleNextDelete = async () => {
        ...
        toast.success(<CustomToast type='success' title='Error' content={glosary.toastDelete} />, { theme: 'colored', icon: false, style: { backgroundColor: '#00C851', maxWidth: 450, padding: 24, borderRadius: 10 } })
    ...
    const handleNextPause = async () => {
        ...
        toast.success(<CustomToast type='success' title='Error' content={glosary.toastPause} />, { theme: 'colored', icon: false, style: { backgroundColor: '#00C851', maxWidth: 450, padding: 24, borderRadius: 10 } })
        await fetchProperties()
        setSelected([])
        store?.setLoading(false)
    }

...
const Modal: React.FC<IModal> = ({ open, onRequestClose, children }) => {
    ...
    return (
        <ReactModal isOpen={open} onRequestClose={onRequestClose} appElement={parent || undefined} style={{
            ...
            content: { backgroundColor: '#FFFFFF', padding: 24, borderRadius: 10, boxShadow: '0px 12px 30px #1973FA14', width: 384, position: 'relative', display: 'flex', flexDirection: 'column', gap:16, color:'#000000DE', inset:0 , zIndex:5}
        }}>
            {children}
        </ReactModal>
    )
